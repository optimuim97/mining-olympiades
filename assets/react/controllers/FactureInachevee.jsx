...
export default function (abonnement) {
    ...
    const styles = StyleSheet.create({
        ...
        attention:{border: 1, marginTop:15, marginLeft: 250, fontSize: 12, paddingTop:10, paddingBottom:10, paddingLeft:7, paddingRight: 7},
        compagnie:{textTransform: "uppercase", marginBottom:5, fontFamily: 'Helvetica'},
        label:{fontSize:10, marginRight:3},
        representant:{fontWeight: "heavy", paddingLeft:10, marginLeft:10, fontFamily: 'Times-BoldItalic'},
        ...
        content:{paddingTop:30, paddingBottom:30, paddingLeft:15, paddingRight:15},
        liste:{fontSize:14},
        discipline:{paddingLeft:20, paddingTop:7, fontStyle: "italic", fontSize:12, fontFamily: 'Helvetica-Oblique'},
        montantTotal:{marginTop:30, marginBottom:30, paddingLeft:15, flexDirection: "row"},
        colonne2:{flexGrow:1},
        ...
        piedPage:{fontFamily: "Times-Bold", fontSize:9, color: "orange", bottom:5, left:30, right:0, position: "absolute", borderTop:2, borderColor: "orange", paddingTop:7, width:200}
    });
            ...
            <Page size="A4" orientation="portrait" style={styles.body}>
                ...
                <View style={styles.attention}>
                    {abonnement.abonnement.compagnie && (
                        <div>
                            <Text style={styles.compagnie}>{abonnement.abonnement.compagnie.titre}</Text>
                            <Text>
                                <Text style={styles.label}>Att:</Text>
                                <Text style={styles.representant}>{abonnement.abonnement.compagnie.representant}</Text>
                            </Text>
                        </div>
                    )}
                </View>
                <Text style={styles.reference}>Facture N°:
                    <Text style={styles.representantValeur}>{abonnement.abonnement.reference}</Text>
                </Text>
                <View style={styles.tableau}>
                        ...
                        { abonnement.abonnement.disciplines && (
                            <div>
                                {abonnement.abonnement.disciplines.map((discipline, index)=> (
                                    <Text key={index} style={styles.discipline}>- {discipline.titre } </Text>
                                ))}
                            </div>
                        )}
                    </View>
                    <View style={styles.montantTotal}>
                        <Text style={styles.colonne2}>
                            <Text style={styles.labelMontantTotal}>Montant total de la partication : </Text>
                        </Text>
                        <Text style={styles.valeurMontantTotal}>{abonnement.abonnement.montant}</Text>
                    </View>
                    <Text style={styles.bgOrange}/>
                </View>
                <View style={styles.sectionReglement}>
                    <Text style={styles.labelReglement}>Vous pouvez effectuer votre règlement:</Text>
                    <Text style={styles.optionReglement}>
                        * par chèque à l'ordre de GPMCI ou Groupement Professionnel des Miniers de Côte d'Ivoire
                    </Text>
                    <Text style={styles.optionReglement}>
                        * par virement bancaire : Groupement Professionnel des Miniers de Côte d'Ivoire (GPMCI)
                        sur le compte N° CI059 01004 120070130001 72 à ECOBANK bd Latrille II Plateaux Vallons.
                    </Text>
                </View>
                <Text style={styles.consigne}>Quel que soit votre mode de règlement, prière contacter le service comptabilité pour la remise du reçu de paiement.</Text>
                ...
                <p>
                    ...
                    <PDFDownloadLink document={<Facture />} fileName={`facture-${abonnement.abonnement.reference}.pdf`}>
                        {({ blob, url, loading, error }) =>
                            loading ? 'Chargement de la facture...' : 'Voulez-vous télécharger votre facture? Veuillez cliquer sur le lien.'
                        }
                    </PDFDownloadLink>
                </p>
            </div>
